...
$OptionBProposedProxyPortStatus = "ACTIVE current Cloudflare target"
...
$PreCutoverLiveChecklistPath = ".\docs\BLUE_GREEN_PRE_CUTOVER_LIVE_CHECKLIST.md"
$PreCutoverLiveChecklistStatus = "READY after review"
$CloudflareCutoverStatus = "PASSED"
$CloudflareProposedCutoverTarget = "http://127.0.0.1:18000"
$CloudflareCurrentRouteTarget = "http://127.0.0.1:18000"
$CloudflareRollbackTarget = "http://127.0.0.1:8000"
$CloudflareManualCutoverApprovalPhrase = "I_APPROVE_MANUAL_CLOUDFLARE_CUTOVER_TO_18000_AFTER_LIVE_CHECKS"
...
$ProductionCandidateProxyStatus = "ACTIVE EXTERNAL TRAFFIC PATH / MUST REMAIN RUNNING"
...
$ProductionCandidateRouteStatus = "PASSED"
...
$FinalRuntimeRehearsalStatus = "PASSED"
...
$TrafficPathOptionRecommendedDirection = "Option B, active after manual Cloudflare cutover"
$TrafficPathOptionChosenStatus = "Option B manual Cloudflare route cutover"
$CloudflareChangeApprovalStatus = "PASSED before this dry-run; no Cloudflare change in this task"
...
$ExternalRoutingRecommendedNextPath = "post-cutover observation and hardening plan"
...
function Show-DraftArtifactSummary {
    ...
    $artifacts = @(
        ...
        [pscustomobject]@{
            Label = "Pre-cutover live checklist"
            Path = $PreCutoverLiveChecklistPath
        },
        [pscustomobject]@{
...
function Show-DeploymentLockStatus {
    ...
    if (Test-Path -LiteralPath $PreCutoverLiveChecklistPath) {
        Write-Ok "Pre-cutover live checklist exists: $PreCutoverLiveChecklistPath"
    } else {
        Write-Warn "Pre-cutover live checklist is missing: $PreCutoverLiveChecklistPath"
    }
    ...
    Write-Host "Pre-cutover live checklist: $PreCutoverLiveChecklistStatus."
    Write-Host "Pre-cutover live checklist exists: $(Test-Path -LiteralPath $PreCutoverLiveChecklistPath)."
    Write-Host "Manual Cloudflare cutover: $CloudflareCutoverStatus."
    ...
    Write-Host "Current route target: $CloudflareCurrentRouteTarget."
    Write-Host "Rollback target: $CloudflareRollbackTarget."
    Write-Host "Candidate services must remain running: $ProductionCandidateProxyService, $ProductionCandidateBlueService, $ProductionCandidateGreenService."
    Write-Host "Production blue-green external traffic path: ACTIVE through $ProductionCandidateProxyPort candidate."
    ...
    Write-Host "Final runtime rehearsal: $FinalRuntimeRehearsalStatus."
    ...
    Write-Host "Production apply scripts remain blocked unless a separate future task approves exact runtime commands behind deployment lock gates."
    Write-Host "No additional Cloudflare/domain routing change is approved without separate approval."
...
function Show-FuturePlan {
    ...
    Write-Host "Pre-cutover live checklist path: $PreCutoverLiveChecklistPath."
    ...
    Write-Host "No additional Cloudflare/domain routing change is approved without separate approval."
    Write-Host "No host port 8000 ownership change is approved without separate approval."
    Write-Host "Production apply scripts remain no-action / blocked unless separately approved."
    Write-Host ""
...
Write-Ok "Pre-cutover live checklist: $PreCutoverLiveChecklistStatus."
Write-Ok "Pre-cutover live checklist exists: $(Test-Path -LiteralPath $PreCutoverLiveChecklistPath)."
Write-Ok "Manual Cloudflare cutover: $CloudflareCutoverStatus."
...
Write-Ok "Current route target: $CloudflareCurrentRouteTarget."
Write-Ok "Rollback target: $CloudflareRollbackTarget."
Write-Ok "Candidate services must remain running: $ProductionCandidateProxyService, $ProductionCandidateBlueService, $ProductionCandidateGreenService."
Write-Ok "Production blue-green external traffic path: ACTIVE through $ProductionCandidateProxyPort candidate."
...
Write-Ok "18000 candidate route: $ProductionCandidateRouteStatus."
Write-Ok "Option B proxy candidate local path: $ProductionCandidateValidationStatus."
Write-Ok "Final runtime rehearsal: $FinalRuntimeRehearsalStatus."
...
Write-Ok "Production blue-green real apply remains blocked unless a separate future phase approves exact runtime commands behind deployment lock gates."
Write-Ok "No additional Cloudflare/domain routing change is approved without separate approval."
