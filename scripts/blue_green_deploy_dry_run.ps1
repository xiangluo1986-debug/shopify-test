...
$NextBlueGreenStep = "fill manual Cloudflare dashboard routing fields before choosing the production origin implementation path"
...
$ExternalRoutingRecommendedNextPath = "manual Cloudflare dashboard check fields to fill"
$ProductionProxyOwnershipStatus = "manual decision required; origin target unconfirmed"
...
function Show-DeploymentLockStatus {
    ...
    Write-Host "Production apply remains blocked until the manual Cloudflare dashboard fields identify the exact origin target, external routing is confirmed, and exact runtime commands based on the documented defaults are implemented, reviewed, approved, and covered by manual production approval."
...
function Show-FuturePlan {
    ...
    Write-Host "Production apply remains blocked until the manual Cloudflare dashboard fields identify the exact origin target, external routing is confirmed, and exact runtime commands based on the documented defaults are implemented, reviewed, approved, and covered by manual approval."
    Write-Host "No Cloudflare/domain routing change is approved without separate approval."
    Write-Host "No host port 8000 ownership change is approved without separate approval."
...
Write-Ok "Production blue-green real apply remains NO-GO until the manual Cloudflare dashboard fields identify the exact origin target and a separate future phase approves exact runtime commands behind deployment lock gates."
