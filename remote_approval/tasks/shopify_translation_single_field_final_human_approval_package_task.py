...
TASK_NAME = "shopify_translation_single_field_final_human_approval_package"
COMMAND_LABEL = "shopify_translation_single_field_final_human_approval_package"
...
FINAL_HUMAN_APPROVAL_JSON_PATH = LOG_DIR / "shopify_translation_single_field_final_human_approval_package.json"
FINAL_HUMAN_APPROVAL_HTML_PATH = LOG_DIR / "shopify_translation_single_field_final_human_approval_package.html"
...
EXPECTED_PRE_EXECUTION_TASK = "shopify_translation_single_field_real_write_pre_execution_validate"
EXPECTED_PRE_EXECUTION_MODE = "pre-execution-validation-only"
...
READY_PRE_EXECUTION_STATUSES = {
    "ready_for_manual_write_approval",
    "pre_execution_validation_passed_pending_human_approval",
}
...
def run_shopify_translation_single_field_final_human_approval_package_task(mode: str) -> dict:
    if mode != "dry-run":
    ...
    validators = [
        ...
        ("pre_execution", _validate_pre_execution),
    ]
    ...
    pre_execution_validation_summary = _pre_execution_validation_summary(reports["pre_execution"])
    ...
    approval_package_status = _approval_package_status(blocking_conditions)
    ...
    payload = {
        ...
        "task": TASK_NAME,
        "task_name": TASK_NAME,
        "mode": "final-human-approval-package-only",
        ...
        "json_final_human_approval_package_path": str(FINAL_HUMAN_APPROVAL_JSON_PATH),
        "html_final_human_approval_package_path": str(FINAL_HUMAN_APPROVAL_HTML_PATH),
        "success": success,
        "approval_package_status": approval_package_status,
        ...
        "final_human_approval_checklist": _final_human_approval_checklist(requested_scope, verified_backup_summary),
        "final_human_approval_statement": _final_human_approval_statement(requested_scope),
        "phase_12_entry_requirements": _phase_12_entry_requirements(),
        "phase_12_must_not_skip": _phase_12_must_not_skip(),
        ...
        "final_human_approval_package_only": True,
        ...
        "detected_issue_summary": _issue_summary(approval_package_status, blocking_conditions),
    ...
    return {
        ...
        "json_final_human_approval_package_path": str(json_path),
        "html_final_human_approval_package_path": str(html_path),
        "approval_package_status": approval_package_status,
        "final_human_approval_package_only": True,
        "phase_12_entry_allowed": False,
        "dangerous_flag_present": dangerous_flag_present,
...
def _validate_pre_execution(report: dict) -> list[str]:
    errors = []
    if report.get("task") != EXPECTED_PRE_EXECUTION_TASK or report.get("mode") != EXPECTED_PRE_EXECUTION_MODE:
        errors.append("unsafe_pre_execution_validation_report")
    if report.get("validation_status") not in READY_PRE_EXECUTION_STATUSES:
        errors.append("pre_execution_validation_not_ready")
    if report.get("pre_execution_validation_only") is not True:
        errors.append("unsafe_pre_execution_validation_report")
    if report.get("write_execution_allowed") is True:
        errors.append("source_report_indicates_write_execution_allowed")
    dangerous = report.get("dangerous_flag_validation") or {}
...
def _pre_execution_validation_summary(report: dict) -> dict:
    return {
        "validation_status": report.get("validation_status", "") if report else "",
        "pre_execution_validation_only": bool(report.get("pre_execution_validation_only")) if report else False,
        "dangerous_flag_present": bool((report.get("dangerous_flag_validation") or {}).get("dangerous_flag_present"))
        if report
        else False,
        "dangerous_flag_effective": bool((report.get("dangerous_flag_validation") or {}).get("dangerous_flag_effective"))
        if report
        else False,
...
def _blocking_conditions(
    ...
    if pre_execution_summary["validation_status"] not in READY_PRE_EXECUTION_STATUSES:
        conditions.append("pre_execution_validation_not_ready")
    return _unique(conditions)


def _approval_package_status(blocking_conditions: list[str]) -> str:
    ...
    if "pre_execution_validation_not_ready" in blocking_conditions:
        return "blocked_pre_execution_validation_not_ready"
    return "blocked"


def _final_human_approval_checklist(scope: dict, backup_summary: dict) -> list[str]:
    return [
        f"Manually confirm product_id is correct: {scope.get('product_id', '')}",
        f"Manually confirm locale is correct: {scope.get('locale', '')}",
        f"Manually confirm field is correct: {ALLOWED_FIELD}",
        "Manually confirm proposed_value is correct.",
        "Manually confirm proposed_value is no longer than 60 characters.",
        f"Manually confirm backup_value has been saved: {backup_summary.get('backup_value', '')}",
        "Manually confirm backup_source_is_verified=true.",
        "Manually confirm readback plan has been generated.",
        "Manually confirm rollback plan has been generated.",
        "Manually confirm only Phase 12 may perform the first real write.",
        f"Manually confirm Phase 12 must require {FUTURE_REQUIRED_FLAG} again.",
        "Manually confirm Phase 12 must immediately read back after writing.",
        "Manually confirm rollback must not run automatically and requires separate approval.",
    ]


def _final_human_approval_statement(scope: dict) -> str:
    return (
        "I understand the next phase may perform a real Shopify translationsRegister write for exactly "
        "1 product x 1 locale x 1 field=meta_title. I confirm the product ID, locale, field, proposed "
        "value, verified backup, readback plan, and rollback plan are correct. I understand this Phase "
        "11.10 does not write Shopify. "
        f"Scope: product_id={scope.get('product_id', '')}, locale={scope.get('locale', '')}, "
        f"field={scope.get('field', '')}."
    )


def _phase_12_entry_requirements() -> list[str]:
    return [
        "Phase 12 must be an independent task.",
        "Phase 12 must re-read the final human approval package.",
        "Phase 12 must re-verify the verified backup.",
        "Phase 12 must re-verify pre-execution validation.",
        f"Phase 12 must re-verify the dangerous flag: {FUTURE_REQUIRED_FLAG}.",
        "Phase 12 must limit scope to 1 product x 1 locale x 1 field=meta_title.",
        "Phase 12 must immediately read back after the real write.",
        "Phase 12 must not automatically rollback.",
        "Phase 12 must not allow batch mode.",
        "Phase 12 must not scan the whole store.",
        "Phase 12 must not allow multiple locales or fields.",
    ]


def _phase_12_must_not_skip() -> list[str]:
    return [
        "Do not skip backup.",
        "Do not skip final gate.",
        "Do not skip pre-execution validation.",
        "Do not skip final human approval.",
        "Do not skip dangerous flag.",
...
def _write_json_report(payload: dict) -> Path:
    ...
    json.loads(text)
    FINAL_HUMAN_APPROVAL_JSON_PATH.write_text(text, encoding="utf-8")
    json.loads(FINAL_HUMAN_APPROVAL_JSON_PATH.read_text(encoding="utf-8"))
    return FINAL_HUMAN_APPROVAL_JSON_PATH


def _write_html_report(payload: dict) -> Path:
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    FINAL_HUMAN_APPROVAL_HTML_PATH.write_text(_render_html_report(payload), encoding="utf-8")
    return FINAL_HUMAN_APPROVAL_HTML_PATH
...
def _render_html_report(payload: dict) -> str:
    ...
    summary_rows = "\n".join(
        ...
        for label, key in [
            ...
            ("Mode", "mode"),
            ("Timestamp", "timestamp"),
            ("Approval Package Status", "approval_package_status"),
            ("Dangerous Flag", "dangerous_flag_summary"),
            ("Final Human Approval Package Only", "final_human_approval_package_only"),
    ...
    detail_rows = "\n".join(
        ...
        for label, value in [
            ...
            ("Final Human Approval Checklist", payload.get("final_human_approval_checklist", [])),
            ("Final Human Approval Statement", payload.get("final_human_approval_statement", "")),
            ("Phase 12 Entry Requirements", payload.get("phase_12_entry_requirements", [])),
            ("Phase 12 Must Not Skip", payload.get("phase_12_must_not_skip", [])),
...
<head>
  <meta charset="utf-8">
  <title>Shopify Single-Field Final Human Approval Package</title>
  <style>
...
<body>
  <h1>Shopify Single-Field Final Human Approval Package</h1>
  ...
  <ul>
    ...
    <li>This package does not grant Phase 12 entry automatically; a later separate phase must be explicitly approved.</li>
  </ul>
</body>
</html>
"""
...
def _issue_summary(approval_package_status: str, blocking_conditions: list[str]) -> str:
    if blocking_conditions:
        return "Single-field final human approval package blocked: " + ", ".join(blocking_conditions)
    return (
        f"Single-field final human approval package generated with status {approval_package_status}. "
...
def _build_approval_message(payload: dict, json_path: Path, html_path: Path) -> str:
    return (
        "Shopify single-field final human approval package generated.\n"
        f"Approval package status: {payload.get('approval_package_status')}\n"
        ...
        "Final human approval JSON:\n"
        f"{json_path}\n\n"
        "Final human approval HTML:\n"
        f"{html_path}\n"
        "Final human approval package only. No Shopify API call, command execution, readback, rollback, mutation, translationsRegister, or write was performed.\n\n"
        "Allowed actions only:\n"
        "Y / 1 = keep final approval files\n"
