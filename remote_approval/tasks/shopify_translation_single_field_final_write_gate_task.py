...
TASK_NAME = "shopify_translation_single_field_final_write_gate"
COMMAND_LABEL = "shopify_translation_single_field_final_write_gate"
...
FINAL_WRITE_GATE_JSON_PATH = LOG_DIR / "shopify_translation_single_field_final_write_gate.json"
FINAL_WRITE_GATE_HTML_PATH = LOG_DIR / "shopify_translation_single_field_final_write_gate.html"
...
def run_shopify_translation_single_field_final_write_gate_task(mode: str) -> dict:
    if mode != "dry-run":
    ...
    try:
        preflight = _read_json(SOURCE_PREFLIGHT_PACKAGE_PATH)
    ...
    try:
        backup = _read_json(SOURCE_BACKUP_FETCH_PATH)
    except FileNotFoundError as exc:
    ...
    try:
        plan = _read_json(SOURCE_READBACK_ROLLBACK_PLAN_PATH)
    except FileNotFoundError as exc:
        parse_errors.append(f"Readback rollback plan JSON not found: {exc}")
        validation_errors.append("missing_readback_rollback_plan")
    except (OSError, json.JSONDecodeError) as exc:
        parse_errors.append(f"Could not parse readback rollback plan JSON: {exc}")
        validation_errors.append("readback_rollback_plan_json_invalid")
    ...
    if plan:
        ...
        validation_errors.extend(errors)
        validation_warnings.extend(warnings)
    if preflight and backup and plan:
        validation_errors.extend(_validate_scope_match(preflight, backup, plan))

    requested_scope = _requested_scope(preflight, backup, plan)
    proposed_change = _proposed_change(preflight, requested_scope)
    verified_backup_summary = _verified_backup_summary(backup)
    blocking_conditions = _blocking_conditions(validation_errors, proposed_change, verified_backup_summary, plan)
    final_gate_status = (
        "ready_for_human_final_approval"
        if not blocking_conditions
        else _blocked_status(blocking_conditions)
    )
    ...
    payload = {
        ...
        "task": TASK_NAME,
        "task_name": TASK_NAME,
        "mode": "final-write-gate-package-only",
        ...
        "json_final_write_gate_path": str(FINAL_WRITE_GATE_JSON_PATH),
        "html_final_write_gate_path": str(FINAL_WRITE_GATE_HTML_PATH),
        "success": success,
        ...
        "readback_plan_summary": _readback_plan_summary(plan),
        "rollback_plan_summary": _rollback_plan_summary(plan),
        "final_manual_checklist": _final_manual_checklist(requested_scope),
        "dangerous_action_summary": _dangerous_action_summary(),
        "required_future_command_constraints": _required_future_command_constraints(),
        "required_future_human_approval": {
            "required": True,
            "approval_must_be_explicit": True,
            "dangerous_flag_required": "--i-understand-this-writes-shopify",
            "current_phase_grants_write_permission": False,
        },
        ...
        "detected_issue_summary": _issue_summary(final_gate_status, blocking_conditions),
    ...
    return {
        ...
        "json_final_write_gate_path": str(json_path),
        "html_final_write_gate_path": str(html_path),
        "final_gate_status": final_gate_status,
        "backup_source_is_verified": verified_backup_summary["backup_source_is_verified"],
        "read_only_shopify_query_performed": verified_backup_summary["read_only_shopify_query_performed"],
        "final_real_write_allowed": False,
        "real_write_allowed": False,
        "shopify_api_call_performed": False,
        "shopify_write_performed": False,
...
def _validate_plan(report: dict) -> tuple[list[str], list[str]]:
    ...
    proposed = report.get("proposed_change") or {}
    errors.extend(_validate_scope(proposed))
    ...
    for field, condition in [
        ("shopify_api_call_performed", True),
        ("readback_performed", True),
        ("rollback_performed", True),
    ]:
        if report.get(field) is condition:
            errors.append("source_report_indicates_write_or_mutation")
    return _unique(errors), _unique(warnings)
...
def _validate_scope_match(preflight: dict, backup: dict, plan: dict) -> list[str]:
    errors = []
    scopes = [
...
def _requested_scope(preflight: dict, backup: dict, plan: dict) -> dict:
...
def _readback_plan_summary(plan: dict) -> dict:
    readback = plan.get("readback_plan") or {}
    return {
        "readback_required": bool(readback.get("readback_required")),
        "readback_scope": readback.get("readback_scope", {}),
        "future_must_compare_shopify_value_to_proposed_value": bool(
            readback.get("future_must_compare_shopify_value_to_proposed_value")
        ),
        "comparison_failure_status": readback.get("comparison_failure_status", "write_verification_failed"),
        "readback_performed_in_this_phase": False,
    }


def _rollback_plan_summary(plan: dict) -> dict:
    rollback = plan.get("rollback_plan") or {}
    return {
        "rollback_required": bool(rollback.get("rollback_required")),
        "rollback_plan_status": rollback.get("rollback_plan_status", ""),
        "rollback_scope": rollback.get("rollback_scope", {}),
        "rollback_value_present": bool(rollback.get("rollback_value_present")),
        "rollback_value_chars": int(rollback.get("rollback_value_chars") or 0),
        "rollback_uses_verified_backup_value_only": bool(rollback.get("rollback_uses_verified_backup_value_only")),
...
def _blocking_conditions(
    ...
    mapping = {
        ...
        "readback_rollback_plan_not_ready": "readback/rollback plan not ready",
    }
    ...
    if plan and plan.get("plan_status") not in READY_PLAN_STATUSES:
        conditions.append("readback/rollback plan not ready")
...
def _final_manual_checklist(scope: dict) -> list[str]:
    return [
        ...
        "Manually confirm verified backup value has been saved.",
        "Manually confirm rollback plan has been generated.",
        "Manually confirm readback plan has been generated.",
        "Manually confirm only the next separate phase may perform a real write.",
        "Manually confirm the next phase must explicitly provide --i-understand-this-writes-shopify.",
    ]


def _dangerous_action_summary() -> dict:
    return {
        "current_phase_real_write_performed": False,
        "future_real_write_would_call": "Shopify translationsRegister",
        "future_scope_limit": "1 product x 1 locale x 1 field=meta_title",
        "future_requires_immediate_readback_verification": True,
        "readback_failure_requires_verified_backup_rollback_review": True,
        "rollback_requires_separate_approval": True,
    }


def _required_future_command_constraints() -> dict:
    return {
        "must_specify_product_id": True,
        "must_specify_locale": True,
        "must_specify_field_meta_title": True,
        "must_specify_proposed_value": True,
        "must_specify_dangerous_flag": "--i-understand-this-writes-shopify",
        "batch_mode_allowed": False,
        "automatic_product_scan_allowed": False,
        "multiple_locales_allowed": False,
        "multiple_fields_allowed": False,
        "skip_readback_allowed": False,
        "write_without_verified_backup_allowed": False,
    }
...
def _write_json_report(payload: dict) -> Path:
    ...
    json.loads(text)
    FINAL_WRITE_GATE_JSON_PATH.write_text(text, encoding="utf-8")
    json.loads(FINAL_WRITE_GATE_JSON_PATH.read_text(encoding="utf-8"))
    return FINAL_WRITE_GATE_JSON_PATH


def _write_html_report(payload: dict) -> Path:
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    FINAL_WRITE_GATE_HTML_PATH.write_text(_render_html_report(payload), encoding="utf-8")
    return FINAL_WRITE_GATE_HTML_PATH
...
def _render_html_report(payload: dict) -> str:
    ...
    summary_rows = "\n".join(
        ...
        for label, key in [
            ...
            ("Mode", "mode"),
            ("Timestamp", "timestamp"),
            ("Final Gate Status", "final_gate_status"),
    ...
    detail_rows = "\n".join(
        ...
        for label, value in [
            ...
            ("Readback Plan Summary", payload.get("readback_plan_summary", {})),
            ("Rollback Plan Summary", payload.get("rollback_plan_summary", {})),
            ("Dangerous Action Summary", payload.get("dangerous_action_summary", {})),
            ("Required Future Command Constraints", payload.get("required_future_command_constraints", {})),
            ("Safety Summary", payload.get("safety_summary", {})),
        ]
    )
    checklist_items = "\n".join(
        f"<li>{escape(str(item))}</li>" for item in payload.get("final_manual_checklist", [])
    )
    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Shopify Single-Field Final Write Gate</title>
  <style>
...
<body>
  <h1>Shopify Single-Field Final Write Gate</h1>
  ...
  <ul>
    ...
    <li>This package does not allow real writes; a later separate task with the dangerous flag is required.</li>
  </ul>
</body>
</html>
"""
...
def _issue_summary(final_gate_status: str, blocking_conditions: list[str]) -> str:
    if blocking_conditions:
        return "Single-field final write gate blocked: " + ", ".join(blocking_conditions)
    return (
        f"Single-field final write gate package generated with status {final_gate_status}. "
        "No Shopify API calls or writes performed."
    )


def _build_approval_message(payload: dict, json_path: Path, html_path: Path) -> str:
    return (
        "Shopify single-field final write gate package generated.\n"
        f"Final gate status: {payload.get('final_gate_status')}\n"
        ...
        "Final write gate JSON:\n"
        f"{json_path}\n\n"
        "Final write gate HTML:\n"
        f"{html_path}\n"
        "Final gate package only. No Shopify API call, readback, rollback, mutation, translationsRegister, or write was performed.\n\n"
        "Allowed actions only:\n"
        "Y / 1 = keep final gate package files\n"
