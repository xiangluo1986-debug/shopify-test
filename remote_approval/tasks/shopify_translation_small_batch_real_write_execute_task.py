...
TASK_NAME = "shopify_translation_small_batch_real_write_execute"
PHASE = "16.5"
...
GATE_PREFLIGHT_PATH = LOG_DIR / "shopify_translation_small_batch_real_write_gate_preflight.json"
JSON_REPORT_PATH = LOG_DIR / "shopify_translation_small_batch_real_write_execute.json"
HTML_REPORT_PATH = LOG_DIR / "shopify_translation_small_batch_real_write_execute.html"
...
SMALL_BATCH_ONLY_ENV = "SHOPIFY_TRANSLATION_REAL_WRITE_SMALL_BATCH_ONLY"
BATCH_LOCALES_ENV = "SHOPIFY_TRANSLATION_REAL_WRITE_BATCH_LOCALES"
BATCH_FIELD_ENV = "SHOPIFY_TRANSLATION_REAL_WRITE_BATCH_FIELD"


def run_shopify_translation_small_batch_real_write_execute_task(mode: str) -> dict:
    if mode not in SUPPORTED_MODES:
        raise ValueError(f"{TASK_NAME} only supports dry-run, real-run, or execute-real-write mode.")

    started = time.time()
    settings = _read_settings(mode)
    gate_report, gate_diag = _read_json(GATE_PREFLIGHT_PATH)
    gate_conditions = _gate_report_blocking_conditions(gate_report, gate_diag)
    settings["locked_target_entries"] = _locked_entries_from_gate_report(gate_report)
    settings["gate_conditions"] = gate_conditions

    if gate_conditions:
        docker_result = {
            "success": False,
            "failure_type": "gate_preflight_not_ready",
            "execution_status": "blocked_small_batch_gate_preflight_not_ready",
    ...
    success = (
        payload["execution_status"] == "small_batch_real_write_dry_run_ready"
        if payload["dry_run"]
        else payload["execution_status"] == "small_batch_real_write_succeeded_and_verified"
    ...
    return {
        ...
        "json_small_batch_real_write_execute_path": str(json_path),
        "html_small_batch_real_write_execute_path": str(html_path),
        "phase": PHASE,
        ...
        "requested_batch_locales": payload["requested_batch_locales"],
        "requested_batch_field": payload["requested_batch_field"],
        "small_batch_selected_count": payload["small_batch_selected_count"],
...
def _read_settings(mode: str) -> dict:
    ...
    batch_locales_raw = os.environ.get(BATCH_LOCALES_ENV, "").strip()
    batch_field_raw = os.environ.get(BATCH_FIELD_ENV, "").strip()
    small_batch_only_raw = os.environ.get(SMALL_BATCH_ONLY_ENV, "").strip()
    ...
    settings = {
        ...
        "max_entries": _parse_int(max_entries_raw) if max_entries_raw else LOCKED_BATCH_MAX_ENTRIES,
        "env_max_entries": _parse_int(max_entries_raw) if max_entries_raw else None,
        "small_batch_only": small_batch_only_raw == "1" if small_batch_only_raw else False,
        "small_batch_only_raw": small_batch_only_raw,
        "requested_batch_locales": _split_csv(batch_locales_raw) or LOCKED_BATCH_LOCALES,
        "env_batch_locales": _split_csv(batch_locales_raw),
        "requested_batch_field": batch_field_raw or LOCKED_BATCH_FIELD,
        "env_batch_field": batch_field_raw,
    }
...
def _settings_blocking_conditions(settings: dict) -> list[str]:
    ...
    if settings.get("env_product_id") and settings["env_product_id"] != PRODUCT_ID:
        conditions.append("small_batch_product_id_mismatch")
    if settings.get("env_max_entries") is not None and settings["env_max_entries"] != LOCKED_BATCH_MAX_ENTRIES:
        conditions.append("small_batch_max_entries_not_three")
    if settings.get("small_batch_only_raw") and not settings.get("small_batch_only"):
        conditions.append("small_batch_only_not_enabled")
    if settings.get("env_batch_locales") and settings["env_batch_locales"] != LOCKED_BATCH_LOCALES:
        conditions.append("small_batch_batch_locales_mismatch")
    if settings.get("env_batch_field") and settings["env_batch_field"] != LOCKED_BATCH_FIELD:
        conditions.append("small_batch_batch_field_mismatch")
    if real_run_requested:
        if not settings.get("ack_matches"):
            conditions.append("small_batch_ack_missing_or_invalid")
        if settings.get("env_product_id") != PRODUCT_ID:
            conditions.append("small_batch_product_id_required")
        if not settings.get("small_batch_only"):
            conditions.append("small_batch_only_not_enabled")
        if settings.get("env_max_entries") != LOCKED_BATCH_MAX_ENTRIES:
            conditions.append("small_batch_max_entries_not_three")
        if settings.get("env_batch_locales") != LOCKED_BATCH_LOCALES:
            conditions.append("small_batch_batch_locales_mismatch")
        if settings.get("env_batch_field") != LOCKED_BATCH_FIELD:
            conditions.append("small_batch_batch_field_mismatch")
...
def _build_payload(
    ...
    blocking_conditions = _unique(gate_conditions + env_conditions + docker_conditions)
    if failure_type and failure_type not in {"gate_preflight_not_ready"}:
        blocking_conditions = _unique(blocking_conditions + [failure_type])

    if gate_conditions:
        execution_status = "blocked_small_batch_gate_preflight_not_ready"
    elif env_conditions:
        execution_status = "blocked_small_batch_env_scope_mismatch"
    elif failure_type:
        execution_status = "blocked_current_small_batch_executor_scan_failed"
    else:
        execution_status = docker_result.get("execution_status", "")

    dry_run = bool(settings.get("dry_run", True))
    if dry_run and not blocking_conditions and execution_status == "small_batch_real_write_dry_run_ready":
        audit_status = "small_batch_real_write_audit_not_run_dry_run"
    else:
        audit_status = docker_result.get("audit_status", "")
        if not audit_status:
            audit_status = "small_batch_real_write_audit_not_run_dry_run" if dry_run else "small_batch_real_write_audit_not_run"
    ...
    payload = {
        ...
        "requested_batch_locales": settings["requested_batch_locales"],
        "requested_batch_field": settings["requested_batch_field"],
        "small_batch_only": bool(settings.get("small_batch_only")),
        "max_entries": int(settings.get("max_entries") or 0),
        "source_gate_preflight_report_path": str(GATE_PREFLIGHT_PATH),
        "source_gate_preflight_report_exists": bool(gate_diag.get("file_exists")),
        "source_gate_preflight_report_error": gate_diag.get("error", ""),
        "source_gate_preflight_status": gate_report.get("small_batch_real_write_gate_status", ""),
        "source_gate_locked_ready": gate_report.get("locked_small_batch_real_write_ready"),
        "locked_small_batch_real_write_ready": bool(
            gate_report.get("locked_small_batch_real_write_ready") is True
            and not gate_conditions
        ),
        "small_batch_selected_entries": docker_result.get("small_batch_selected_entries", []),
        "small_batch_selected_count": int(docker_result.get("small_batch_selected_count") or 0),
        ...
        "post_write_readback_expected_count": LOCKED_BATCH_MAX_ENTRIES,
        ...
        "manual_small_batch_real_write_allowed_next_step": bool(
            dry_run
            and execution_status == "small_batch_real_write_dry_run_ready"
            and not blocking_conditions
            and int(docker_result.get("small_batch_selected_count") or 0) == LOCKED_BATCH_MAX_ENTRIES
            and int(docker_result.get("translations_register_payload_count") or 0) == LOCKED_BATCH_MAX_ENTRIES
        ),
        "blocking_conditions": blocking_conditions,
        "env_blocking_conditions": env_conditions,
        "gate_preflight_blocking_conditions": gate_conditions,
        "small_batch_real_write_command_powershell": _real_write_command_preview(),
        "post_small_batch_real_write_check_command_powershell": _post_write_check_command(),
...
LOCKED_BATCH_LOCALES = {LOCKED_BATCH_LOCALES!r}
LOCKED_BATCH_FIELD = {LOCKED_BATCH_FIELD!r}
LOCKED_BATCH_MAX_ENTRIES = {LOCKED_BATCH_MAX_ENTRIES!r}
...
result = {{
    ...
    "audit_status": "",
    "blocking_conditions": [],
    "small_batch_selected_entries": [],
    "small_batch_selected_count": 0,
...
if not installation:
    ...
    result["execution_status"] = "blocked_current_small_batch_executor_scan_failed"
    result["audit_status"] = "small_batch_real_write_audit_not_run"
    ...
    for target in [
        ...
        if len(candidates) == 0:
            blocking.append("small_batch_target_missing")
        elif len(candidates) > 1:
            blocking.append("small_batch_target_not_unique")
        else:
            selected.append(candidates[0])
    if len(selected) != LOCKED_BATCH_MAX_ENTRIES:
        blocking.append("small_batch_selected_count_not_three")

    for entry in selected:
        locked = locked_digest(entry.get("locale"), entry.get("field"))
        if not locked or entry.get("digest") != locked:
            entry["blocking_reasons"] = _unique(list(entry.get("blocking_reasons") or []) + ["small_batch_digest_changed"])
            blocking.append("small_batch_digest_changed")
        if entry.get("field") != LOCKED_BATCH_FIELD:
            entry["blocking_reasons"] = _unique(list(entry.get("blocking_reasons") or []) + ["small_batch_unexpected_field"])
            blocking.append("small_batch_unexpected_field")
        if entry.get("locale") not in LOCKED_BATCH_LOCALES:
            entry["blocking_reasons"] = _unique(list(entry.get("blocking_reasons") or []) + ["small_batch_unexpected_locale"])
            blocking.append("small_batch_unexpected_locale")
        planned_value = entry.get("planned_value") or entry.get("proposed_translation") or ""
        if len(planned_value) > 60:
            entry["blocking_reasons"] = _unique(list(entry.get("blocking_reasons") or []) + ["small_batch_meta_title_over_60_chars"])
            blocking.append("small_batch_meta_title_over_60_chars")

    precheck = _pre_write_readback(installation, product_id, LOCKED_BATCH_LOCALES, selected)
    ...
    for entry in selected:
        ...
        if state.get("existing_translation_present"):
            blocking.append("small_batch_existing_translation")
    ...
    if selected_count != LOCKED_BATCH_MAX_ENTRIES:
        blocking.append("small_batch_selected_count_not_three")
    if payload_count != LOCKED_BATCH_MAX_ENTRIES:
        blocking.append("small_batch_payload_count_not_three")
    ...
    real_write_allowed = bool(
        ...
        and settings.get("small_batch_only") is True
        and settings.get("env_max_entries") == LOCKED_BATCH_MAX_ENTRIES
        and settings.get("env_batch_locales") == LOCKED_BATCH_LOCALES
        and settings.get("env_batch_field") == LOCKED_BATCH_FIELD
        and selected_count == LOCKED_BATCH_MAX_ENTRIES
        and payload_count == LOCKED_BATCH_MAX_ENTRIES
    )
    if real_run_requested and not real_write_allowed:
        blocking.append("small_batch_real_write_preconditions_not_met")
    ...
    if real_write_allowed:
        ...
        postcheck = _post_write_readback(installation, product_id, LOCKED_BATCH_LOCALES, selected)
    ...
    result["small_batch_selected_entries"] = [safe_entry(entry) for entry in selected]
    result["small_batch_selected_count"] = selected_count
    result["translations_register_payload_count"] = payload_count
    if settings.get("dry_run") is True:
        result["execution_status"] = (
            "small_batch_real_write_dry_run_ready"
            if not result["blocking_conditions"] and selected_count == LOCKED_BATCH_MAX_ENTRIES and payload_count == LOCKED_BATCH_MAX_ENTRIES
            else "blocked_small_batch_real_write_dry_run_not_ready"
        )
        result["audit_status"] = "small_batch_real_write_audit_not_run_dry_run"
    elif mutation_called and result["verified_count"] == LOCKED_BATCH_MAX_ENTRIES and not result["blocking_conditions"]:
        result["execution_status"] = "small_batch_real_write_succeeded_and_verified"
        result["audit_status"] = "small_batch_real_write_audit_passed"
    elif mutation_called:
        result["execution_status"] = "small_batch_real_write_failed_or_unverified"
        result["audit_status"] = "small_batch_real_write_audit_failed_or_needs_review"
        result["rollback_approval_required"] = True
    else:
        result["execution_status"] = "blocked_small_batch_real_write_preconditions_failed"
        result["audit_status"] = "small_batch_real_write_audit_not_run"
    result["success"] = result["execution_status"] in {{
        "small_batch_real_write_dry_run_ready",
        "small_batch_real_write_succeeded_and_verified",
    }}
...
def _gate_report_blocking_conditions(report: dict, diag: dict) -> list[str]:
    if not diag.get("file_exists"):
        return ["missing_small_batch_real_write_gate_preflight_report"]
    if diag.get("error"):
        return [f"small_batch_real_write_gate_preflight_report_{diag['error']}"]
    expected = {
        "small_batch_real_write_gate_status": "small_batch_real_write_preflight_ready",
        "manual_small_batch_real_write_allowed_next_step": True,
        "locked_small_batch_real_write_ready": True,
    ...
    target_entries = report.get("locked_small_batch_target_entries") or []
    target_pairs = [(entry.get("locale"), entry.get("field")) for entry in target_entries]
    expected_pairs = [(locale, LOCKED_BATCH_FIELD) for locale in LOCKED_BATCH_LOCALES]
    if sorted(target_pairs) != sorted(expected_pairs) or len(target_entries) != LOCKED_BATCH_MAX_ENTRIES:
        conditions.append("locked_small_batch_target_entries_mismatch")
    return _unique(conditions)


def _locked_entries_from_gate_report(report: dict) -> list[dict]:
    entries = []
    source_entries = report.get("locked_small_batch_target_entries") or []
    for locale in LOCKED_BATCH_LOCALES:
        match = next(
            (
                entry
                for entry in source_entries
                if entry.get("locale") == locale and entry.get("field") == LOCKED_BATCH_FIELD
            ),
            {},
        )
        entries.append(
            {
                ...
                "locale": locale,
                "field": LOCKED_BATCH_FIELD,
                "key": match.get("key") or LOCKED_BATCH_FIELD,
...
def _post_write_check_command() -> str:
    keys = [
        ...
        "rollback_performed",
        "blocking_conditions",
        "requested_batch_locales",
        "requested_batch_field",
        "small_batch_selected_count",
    ]
    keys_literal = repr(keys)
    return (
        "python -c \"import json; "
        "p='logs/shopify_translation_small_batch_real_write_execute.json'; "
...
def _render_html(payload: dict) -> str:
    summary_rows = "\n".join(
        ...
        for label, key in [
            ...
            ("Requested Batch Locales", "requested_batch_locales"),
            ("Requested Batch Field", "requested_batch_field"),
            ("Selected Count", "small_batch_selected_count"),
    ...
    entry_rows = "\n".join(
        ...
        "</tr>"
        for entry in payload.get("small_batch_selected_entries", [])
    )
    command_rows = "\n".join(
        f"<li><code>{escape(line)}</code></li>"
        for line in payload.get("small_batch_real_write_command_powershell", [])
    )
    return f"""<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>Small Batch Real-Write Execute</title></head>
<body>
  <h1>Small Batch Real-Write Execute</h1>
  <p>Phase 16.5. Dry-run mode is no-write. Real-run is gated by exact ACK, product, locale, field, digest, and count checks, then requires immediate readback verification.</p>
  <h2>Summary</h2>
  ...
  <table border="1" cellspacing="0" cellpadding="6">
    <thead><tr><th>Locale</th><th>Field</th><th>Key</th><th>Digest</th><th>Pre-Write Digest</th><th>Chars</th><th>Would Write</th><th>Write Performed</th><th>Verified</th><th>Blocking Conditions</th></tr></thead>
    <tbody>{entry_rows}</tbody>
  </table>
  <h2>Future Real-Write Command Preview</h2>
  <p>Do not run this in Phase 16.5 validation. It is a preview for a later explicit manual real-run.</p>
  <ol>{command_rows}</ol>
</body>
</html>
"""
...
def _approval_message(payload: dict, json_path: Path, html_path: Path) -> str:
    return (
        "Phase 16.5 locked small-batch real-write executor report generated.\n"
        f"- execution_status: {payload.get('execution_status')}\n"
        f"- audit_status: {payload.get('audit_status')}\n"
        f"- dry_run: {payload.get('dry_run')}\n"
        f"- small_batch_selected_count: {payload.get('small_batch_selected_count')}\n"
