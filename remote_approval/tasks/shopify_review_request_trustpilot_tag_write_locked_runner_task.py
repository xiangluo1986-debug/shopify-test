...
TASK_NAME = "shopify_review_request_trustpilot_tag_write_locked_runner"
COMMAND_LABEL = "shopify_review_request_trustpilot_tag_write_locked_runner"

SOURCE_DESIGN_JSON_PATH = LOG_DIR / "shopify_review_request_trustpilot_tag_write_design_dry_run.json"
SOURCE_DESIGN_HTML_PATH = LOG_DIR / "shopify_review_request_trustpilot_tag_write_design_dry_run.html"
PROTECTED_DRAFT_SOURCE_JSON_PATH = LOG_DIR / "shopify_review_request_trustpilot_gmail_one_draft_locked_runner.json"
REPORT_JSON_PATH = LOG_DIR / "shopify_review_request_trustpilot_tag_write_locked_runner.json"
REPORT_HTML_PATH = LOG_DIR / "shopify_review_request_trustpilot_tag_write_locked_runner.html"

DRY_RUN_STATUS = "dry_run_trustpilot_tag_not_written"
FUTURE_WRITTEN_STATUS = "one_trustpilot_tag_written_and_needs_audit"
EXPECTED_DESIGN_STATUS = "trustpilot_tag_write_design_dry_run_ready"
...
def run_shopify_review_request_trustpilot_tag_write_locked_runner_task(mode: str) -> dict:
    if mode != "dry-run":
    ...
    full_draft_id_leak = _full_draft_id_leak_detected(
        _read_text(SOURCE_DESIGN_JSON_PATH),
        _read_text(SOURCE_DESIGN_HTML_PATH),
    )
    gates = _gate_status(source_design)
    blocking_conditions = _blocking_conditions(source_design, source_error, source_privacy_scan, full_draft_id_leak, gates)
    write_result = _write_result(gates, blocking_conditions)
    status = write_result["tag_write_locked_status"]
...
def _read_source_design() -> tuple[dict, str]:
    if not SOURCE_DESIGN_JSON_PATH.exists():
        return {}, "blocked_missing_tag_write_design_report"
    try:
        return json.loads(SOURCE_DESIGN_JSON_PATH.read_text(encoding="utf-8")), ""
    except json.JSONDecodeError as exc:
        return {}, _sanitize_text(f"blocked_missing_tag_write_design_report: source JSON parse failed: {exc}")
...
def _gate_status(source_design: dict) -> dict:
    ...
    requested_target_order = os.environ.get(TAG_WRITE_TARGET_ORDER_ENV, "").strip() or _safe_text(
        source_design.get("selected_order_name", EXPECTED_ORDER_NAME)
    )
    requested_tag_value = os.environ.get(TAG_WRITE_VALUE_ENV, "").strip() or _safe_text(
        source_design.get("planned_shopify_tag", EXPECTED_TAG_VALUE)
    )
...
def _blocking_conditions(
    ...
    conditions = []
    if source_error:
        return [{"status": "blocked_missing_tag_write_design_report", "detail": _sanitize_text(source_error)}]
    if source_design.get("tag_write_design_status") != EXPECTED_DESIGN_STATUS or source_design.get("success") is not True:
        conditions.append({"status": "blocked_tag_write_design_not_ready", "detail": "Phase 3.18 tag-write design is not ready."})
    if source_design.get("source_send_audit_status") != EXPECTED_SEND_AUDIT_STATUS:
        conditions.append({"status": "blocked_send_audit_not_passed", "detail": "source send audit did not pass."})
    if int(source_design.get("source_sent_count") or 0) != 1:
        conditions.append({"status": "blocked_unexpected_sent_count", "detail": "source_sent_count must equal 1."})
    if source_design.get("duplicate_trustpilot_tag_detected") is True:
        conditions.append({"status": "blocked_duplicate_trustpilot_tag_detected", "detail": "source design detected a duplicate Trustpilot tag."})
    if source_design.get("repeat_customer_guard_confirmed") is not True:
        conditions.append({"status": "blocked_repeat_customer_guard_not_confirmed", "detail": "repeat customer guard is not confirmed."})
    if source_design.get("returned_package_guard_confirmed") is not True:
        conditions.append({"status": "blocked_returned_package_guard_not_confirmed", "detail": "returned package guard is not confirmed."})
    if _safe_text(source_design.get("selected_order_name", "")) != EXPECTED_ORDER_NAME:
        conditions.append({"status": "blocked_target_order_mismatch", "detail": "source selected_order_name mismatch."})
    if _safe_text(source_design.get("selected_masked_email", "")) != EXPECTED_MASKED_EMAIL:
        conditions.append({"status": "blocked_privacy_scan_failed", "detail": "source selected_masked_email mismatch."})
    if _safe_text(source_design.get("source_gmail_draft_id_partial", "")) != EXPECTED_DRAFT_ID_PARTIAL:
        conditions.append({"status": "blocked_privacy_scan_failed", "detail": "source Gmail draft id partial mismatch."})
    if _safe_text(source_design.get("planned_shopify_tag", "")) != EXPECTED_TAG_VALUE:
        conditions.append({"status": "blocked_tag_value_mismatch", "detail": "source planned tag value mismatch."})
    ...
    if not gates["dry_run"]:
        ...
        if gates["all_real_run_gates_valid"]:
            conditions.append(
                {
                    "status": "blocked_real_tag_write_not_enabled_in_phase_3_19",
                    "detail": "Phase 3.19 validates the locked runner only and does not execute Shopify tagsAdd.",
                }
            )
    return conditions
...
def _write_result(gates: dict, blocking_conditions: list[dict]) -> dict:
    result = {
        "tag_write_locked_status": DRY_RUN_STATUS if not blocking_conditions else blocking_conditions[0]["status"],
...
def _build_payload(
    ...
    payload = {
        ...
        "tag_write_locked_status": status,
        "success": dry_run_success,
        "source_design_status": _safe_text(source_design.get("tag_write_design_status", "")),
        "source_report_used": {
        ...
        "selected_order_name": _safe_text(source_design.get("selected_order_name", EXPECTED_ORDER_NAME)),
        "selected_masked_email": _safe_text(source_design.get("selected_masked_email", EXPECTED_MASKED_EMAIL)),
        "source_gmail_draft_id_partial": _safe_text(source_design.get("source_gmail_draft_id_partial", EXPECTED_DRAFT_ID_PARTIAL)),
        "planned_shopify_tag": _safe_text(source_design.get("planned_shopify_tag", EXPECTED_TAG_VALUE)),
        "source_send_audit_status": _safe_text(source_design.get("source_send_audit_status", "")),
        "source_sent_count": int(source_design.get("source_sent_count") or 0),
        "source_duplicate_trustpilot_tag_detected": source_design.get("duplicate_trustpilot_tag_detected") is True,
        "source_repeat_customer_guard_confirmed": source_design.get("repeat_customer_guard_confirmed") is True,
        "source_returned_package_guard_confirmed": source_design.get("returned_package_guard_confirmed") is True,
        ...
        "would_add_tag": not blocking_conditions or (
            len(blocking_conditions) == 1
            and blocking_conditions[0]["status"] == "blocked_real_tag_write_not_enabled_in_phase_3_19"
        ),
        ...
        "future_real_write_status_on_success": FUTURE_WRITTEN_STATUS,
        "future_real_write_design": {
            ...
            "post_write_audit_required_phase": "3.20",
        ...
        "safety_summary": _safety_summary(write_result),
        **_safety_summary(write_result),
        **write_result,
        "json_path": str(REPORT_JSON_PATH),
        "html_path": str(REPORT_HTML_PATH),
        "json_trustpilot_tag_write_locked_runner_path": str(REPORT_JSON_PATH),
        "html_trustpilot_tag_write_locked_runner_path": str(REPORT_HTML_PATH),
...
def _task_result(payload: dict, json_path: Path, html_path: Path) -> dict:
    return {
        ...
        "json_trustpilot_tag_write_locked_runner_path": str(json_path),
        "html_trustpilot_tag_write_locked_runner_path": str(html_path),
        "tag_write_locked_status": payload["tag_write_locked_status"],
        ...
        "would_add_tag": payload["would_add_tag"],
...
<head>
  <meta charset="utf-8">
  <title>Trustpilot Tag Write Locked Runner</title>
  <style>
...
<body>
  <h1>Trustpilot Tag Write Locked Runner</h1>
  <p class="warning">Phase 3.19 is locked dry-run validation only. No Shopify tag was written.</p>
  <p>Status: <strong>{escape(payload["tag_write_locked_status"])}</strong></p>
  ...
  <p>Would add tag: <strong>{escape(str(payload["would_add_tag"]))}</strong></p>
  <p>Would add tag value: <code>{escape(payload["would_add_tag_value"])}</code></p>
...
def _apply_self_privacy_assertion(payload: dict) -> dict:
    ...
    if self_scan["raw_customer_email_count"] or self_scan["token_secret_bearer_pattern_count"]:
        payload["tag_write_locked_status"] = "blocked_privacy_scan_failed"
        payload["success"] = False
        payload["real_tag_write_allowed"] = False
        payload["would_add_tag"] = False
        payload["blocking_conditions"].append(
            {"status": "blocked_privacy_scan_failed", "detail": "tag-write locked runner self privacy scan failed."}
        )
...
def _issue_summary(status: str, blocking_conditions: list[dict]) -> str:
    if status == DRY_RUN_STATUS:
        return "Trustpilot tag-write locked runner stayed dry-run; no Shopify tag was written."
    return "Trustpilot tag-write locked runner blocked: " + ", ".join(
...
def _approval_message(payload: dict, json_path: Path, html_path: Path) -> str:
    return (
        "Shopify review request Phase 3.19 Trustpilot tag-write locked runner finished.\n"
        f"Status: {payload.get('tag_write_locked_status')}\n"
        f"Selected order: {payload.get('selected_order_name')}\n"
        f"Planned tag: {payload.get('planned_shopify_tag')}\n"
        f"Dry-run: {payload.get('dry_run')}\n"
        f"Would add tag: {payload.get('would_add_tag')}\n"
