...
- Small batch apply execute may read `logs/shopify_translation_small_batch_apply_plan_package.json` and validate execution readiness in dry-run / blocking mode only. Phase 13.1 must not call Shopify APIs, call mutations, call `translationsRegister`, perform readback, perform rollback, publish, apply, update, write the database, or git push.
- Small batch apply execute must keep `mode=dry-run` completely no-write and output `execution_status=dry_run_small_batch_write_not_executed` when the plan is ready. `mode=real-run` or `mode=execute-real-write` must be blocked in Phase 13.1 even if `SHOPIFY_TRANSLATION_SMALL_BATCH_EXECUTION_ACK=YES_I_APPROVE_SMALL_BATCH_SHOPIFY_TRANSLATION_WRITE`; it must report `real_write_allowed=false`, `write_execution_allowed=false`, `translations_register_allowed=false`, `shopify_api_call_performed=false`, `shopify_write_performed=false`, `mutation_performed=false`, `translations_register_called=false`, `readback_performed=false`, `rollback_performed=false`, `publish_performed=false`, `bulk_write_performed=false`, `real_apply_performed=false`, `no_new_shopify_writes_performed=true`, and `all_new_actions_no_write_confirmed=true`.
