...
- [ ] CSV/JSON small batch manual real-run test package tasks are manual-test-package-only, read the CSV/JSON plan, small batch execute dry-run, and CSV/JSON readiness reports only, require all preconditions to remain no-write, may show manual real-run and audit commands, and never execute those commands or perform Shopify API calls, writes, mutations, `translationsRegister`, readback, rollback, publish, or apply actions.
- [ ] CSV/JSON small batch post-write audit package tasks are audit-only, read local Phase 14 reports only, require the source execute report to be a successful `plan_source=csv_json` real-run with matched readback, preserve source write facts separately, and never perform new Shopify API calls, writes, mutations, `translationsRegister`, readback, rollback, publish, or apply actions.
- [ ] Shopify Product Translation Console pages are staff-only and read-only, may perform only limited Shopify GraphQL read queries for a selected product or up to 5 search results, and must never call OpenAI, generate translations, call mutations, call `translationsRegister`, write Shopify, publish, apply, rollback, add migrations, auto-scan the store, or expose tokens.
- [ ] Selected product missing translation draft package tasks may use read-only Shopify GraphQL plus OpenAI draft generation for missing `title`, `meta_title`, and `meta_description` translations only, must skip existing current translations, must mark outdated translations for manual review, and must never call Shopify mutations, `translationsRegister`, write Shopify, publish, apply, rollback, overwrite existing translations, add migrations, expose tokens, or push.
- [ ] Selected product draft quality refinement must enforce `title` <= 65 chars, `meta_title` <= 60 chars, and `meta_description` <= 155 chars, run no more than two natural rewrite attempts for over-length drafts, keep mechanical English and CTA/shipping/origin claims out of eligible drafts, and set `eligible_for_apply_plan=true` only when `validation_status=draft_ready_for_manual_review`.
- [ ] Selected product Google SEO checks must record `seo_validation_status`, `seo_notes`, recommended min/max character ranges, model/core keyword/forbidden phrase checks, and keep `eligible_for_apply_plan=true` only when the draft quality gate and SEO gate both pass.
