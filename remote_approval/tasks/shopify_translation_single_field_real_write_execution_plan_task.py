...
from remote_approval.tasks import shopify_translation_single_field_real_write_runner_final_safe_shell_task as shell
from remote_approval.utils import LOG_DIR, utc_now_iso


TASK_NAME = "shopify_translation_single_field_real_write_execution_plan"
COMMAND_LABEL = "shopify_translation_single_field_real_write_execution_plan"
...
EXECUTION_PLAN_JSON_PATH = LOG_DIR / "shopify_translation_single_field_real_write_execution_plan.json"
EXECUTION_PLAN_HTML_PATH = LOG_DIR / "shopify_translation_single_field_real_write_execution_plan.html"

EXPECTED_FINAL_SAFE_SHELL_TASK = "shopify_translation_single_field_real_write_runner_final_safe_shell"
EXPECTED_FINAL_SAFE_SHELL_MODE = "final-safe-shell-only"
READY_FINAL_SAFE_SHELL_STATUSES = {
    "final_safe_shell_ready_for_manual_review",
    "ready_for_phase_12_1_design_review",
}
PLAN_ACK_ENV = "SHOPIFY_TRANSLATION_PHASE_12_1A_PLAN_ACK"


def run_shopify_translation_single_field_real_write_execution_plan_task(mode: str) -> dict:
    if mode != "dry-run":
    ...
    for key, label, path, missing_code, invalid_code in [
        ...
        try:
            reports[key] = shell.base._read_json(path)
    ...
    env_scope = shell.base._env_requested_scope()
    dangerous_flag_value = os.environ.get(shell.base.DANGEROUS_FLAG_ENV, "").strip()
    dangerous_flag_present = bool(dangerous_flag_value)
    dangerous_flag_valid = dangerous_flag_value.lower() == "true"
    final_safe_ack_value = os.environ.get(shell.ACK_ENV, "").strip()
    final_safe_ack_present = bool(final_safe_ack_value)
    final_safe_ack_valid = final_safe_ack_value.lower() == "true"
    plan_ack_value = os.environ.get(PLAN_ACK_ENV, "").strip()
    plan_ack_present = bool(plan_ack_value)
    plan_ack_valid = plan_ack_value.lower() == "true"

    validation_errors.extend(shell.base._validate_env_scope(env_scope))
    ...
    validators = [
        ("preflight", shell.base._validate_preflight),
        ("backup", shell.base._validate_backup),
        ("plan", shell.base._validate_plan),
        ("gate", shell.base._validate_gate),
        ("design", shell.base._validate_design),
        ("locked", shell.base._validate_locked),
        ("pre_execution", shell.base._validate_pre_execution),
        ("final_human", shell._validate_final_human_approval),
        ("final_safe_shell", _validate_final_safe_shell),
    ]
    ...
    for report in reports.values():
        if report:
            validation_errors.extend(shell._validate_source_unlock_flags(report))
    ...
    proposed_change = shell.base._proposed_change(requested_scope)
    verified_backup_summary = shell.base._verified_backup_summary(reports["backup"])
    final_gate_summary = shell.base._final_gate_summary(reports["gate"])
    design_summary = shell.base._design_summary(reports["design"])
    locked_runner_summary = shell.base._locked_runner_summary(reports["locked"])
    pre_execution_validation_summary = shell.base._pre_execution_validation_summary(reports["pre_execution"])
    final_human_approval_summary = shell._final_human_approval_summary(reports["final_human"])
    final_safe_shell_summary = _final_safe_shell_summary(reports["final_safe_shell"])
    ...
    execution_plan_status = _execution_plan_status(blocking_conditions)
    ...
    payload = {
        ...
        "json_execution_plan_path": str(EXECUTION_PLAN_JSON_PATH),
        "html_execution_plan_path": str(EXECUTION_PLAN_HTML_PATH),
        "success": success,
        "execution_plan_status": execution_plan_status,
        "requested_scope": requested_scope,
        "validated_scope": {
            "product_count": 1 if shell.base._valid_product_id(requested_scope.get("product_id", "")) else 0,
            "locale_count": 1 if requested_scope.get("locale") in shell.base.ALLOWED_LOCALES else 0,
            "field_count": 1 if requested_scope.get("field") == shell.base.ALLOWED_FIELD else 0,
            "field": requested_scope.get("field", ""),
            "field_allowed": requested_scope.get("field") == shell.base.ALLOWED_FIELD,
            ...
            "allowed_field": shell.base.ALLOWED_FIELD,
            "allowed_locales": sorted(shell.base.ALLOWED_LOCALES),
        },
        ...
        "dangerous_flag_summary": {
            "dangerous_flag_name": shell.base.FUTURE_REQUIRED_FLAG,
            "dangerous_flag_env": shell.base.DANGEROUS_FLAG_ENV,
        ...
        "phase_12_1a_plan_ack_summary": {
            "ack_env": PLAN_ACK_ENV,
            "ack_present": plan_ack_present,
            "ack_value": plan_ack_value,
            "ack_required": True,
            "ack_effective": False,
            "ack_note": "This Phase 12.1A ack only allows execution plan generation and cannot write Shopify.",
        },
        "real_write_execution_plan": _real_write_execution_plan(),
        "translations_register_payload_preview": _translations_register_payload_preview(requested_scope, reports),
        "readback_plan": _readback_plan(),
        "failure_handling_plan": _failure_handling_plan(),
        "rollback_approval_plan": _rollback_approval_plan(),
        "phase_12_1b_required_inputs": _phase_12_1b_required_inputs(),
        "phase_12_1b_manual_approval_statement": _phase_12_1b_manual_approval_statement(requested_scope),
        "phase_12_1b_forbidden_actions": _phase_12_1b_forbidden_actions(),
        "blocking_conditions": blocking_conditions,
        "safety_summary": _safety_summary(dangerous_flag_present, final_safe_ack_present, plan_ack_present),
        "future_required_flag": shell.base.FUTURE_REQUIRED_FLAG,
        ...
        "validation_failures": shell.base._unique(validation_errors),
        "parse_errors": parse_errors,
        "detected_issue_summary": _issue_summary(execution_plan_status, blocking_conditions),
    ...
    return {
        ...
        "json_execution_plan_path": str(json_path),
        "html_execution_plan_path": str(html_path),
        "execution_plan_status": execution_plan_status,
        "execution_plan_only": True,
        "payload_preview_only": True,
        "phase_12_1b_entry_allowed": False,
...
def _validate_final_safe_shell(report: dict) -> list[str]:
    errors = []
    if report.get("task") != EXPECTED_FINAL_SAFE_SHELL_TASK or report.get("mode") != EXPECTED_FINAL_SAFE_SHELL_MODE:
        errors.append("unsafe_final_safe_shell_report")
    if report.get("final_safe_shell_status") not in READY_FINAL_SAFE_SHELL_STATUSES:
        errors.append("final_safe_shell_not_ready")
    if report.get("final_safe_shell_only") is not True:
        errors.append("unsafe_final_safe_shell_report")
    if report.get("phase_12_1_entry_allowed") is not False or report.get("phase_12_entry_allowed") is not False:
    ...
    safe_ack = report.get("final_safe_shell_ack_summary") or {}
    if safe_ack.get("ack_present") is not True or str(safe_ack.get("ack_value") or "").lower() != "true":
        errors.append("missing_final_safe_shell_ack")
    if safe_ack.get("ack_effective") is not False:
        errors.append("unsafe_final_safe_shell_ack_effective")
    scope = report.get("proposed_change") or report.get("requested_scope") or {}
    errors.extend(shell.base._validate_scope(scope))
    errors.extend(shell.base._validate_proposed_value(scope))
    errors.extend(shell.base._validate_no_write_flags(report))
    ...
    return shell.base._unique(errors)
...
def _source_status_summary(reports: dict) -> dict:
    summary = shell._source_status_summary(
        {
            key: reports[key]
            for key in ["preflight", "backup", "plan", "gate", "design", "locked", "pre_execution", "final_human"]
        }
    )
    summary["final_safe_shell_status"] = (
        reports["final_safe_shell"].get("final_safe_shell_status", "") if reports["final_safe_shell"] else ""
    )
    summary["final_safe_shell_loaded"] = bool(reports["final_safe_shell"])
    return summary


def _final_safe_shell_summary(report: dict) -> dict:
    dangerous = (report.get("dangerous_flag_summary") or {}) if report else {}
    ack = (report.get("final_safe_shell_ack_summary") or {}) if report else {}
    return {
        "final_safe_shell_status": report.get("final_safe_shell_status", "") if report else "",
        "final_safe_shell_only": bool(report.get("final_safe_shell_only")) if report else False,
        ...
        "final_safe_shell_ack_present": bool(ack.get("ack_present")) if report else False,
        "final_safe_shell_ack_effective": bool(ack.get("ack_effective")) if report else False,
...
def _blocking_conditions(
    ...
    mapping = {
        ...
        "missing_plan_ack": "missing_plan_ack",
        ...
        "invalid_plan_ack_value": "invalid_plan_ack_value",
    ...
    if proposed_change["proposed_value_chars"] > shell.base.MAX_PROPOSED_VALUE_CHARS:
    ...
    if final_gate_summary["final_gate_status"] not in shell.base.READY_GATE_STATUSES:
        conditions.append("final_gate_not_ready")
    if design_summary["design_status"] not in shell.base.READY_DESIGN_STATUSES:
        conditions.append("design_not_ready")
    if locked_runner_summary["locked_runner_status"] not in shell.base.READY_LOCKED_STATUSES:
        conditions.append("locked_runner_not_locked")
    if pre_execution_summary["validation_status"] not in shell.base.READY_PRE_EXECUTION_STATUSES:
        conditions.append("pre_execution_validation_not_ready")
    if final_human_summary["approval_package_status"] not in shell.READY_FINAL_HUMAN_APPROVAL_STATUSES:
        conditions.append("final_human_approval_not_ready")
    if final_safe_shell_summary["final_safe_shell_status"] not in READY_FINAL_SAFE_SHELL_STATUSES:
        conditions.append("final_safe_shell_not_ready")
    if final_safe_shell_summary["phase_12_1_entry_allowed"] is not False:
        conditions.append("source_report_indicates_phase_12_1_entry_allowed")
    if final_safe_shell_summary["phase_12_entry_allowed"] is not False:
        conditions.append("source_report_indicates_phase_12_entry_allowed")
    return shell.base._unique(conditions)


def _execution_plan_status(blocking_conditions: list[str]) -> str:
    if not blocking_conditions:
        return "execution_plan_ready_for_manual_review"
    if "missing_plan_ack" in blocking_conditions:
        return "blocked_missing_plan_ack"
    ...
    if "final_safe_shell_not_ready" in blocking_conditions:
        return "blocked_final_safe_shell_not_ready"
    return "blocked"


def _translations_register_payload_preview(scope: dict, reports: dict) -> dict:
    proposed_value = str(scope.get("proposed_value") or "")
    backup = reports.get("backup") or {}
    return {
        "payload_preview_only": True,
        "resourceId": scope.get("product_id", ""),
        ...
        "translatable_content_digest_summary": {
            "digest_available": False,
            "digest_source": "not_available_in_phase_12_1a_local_reports",
            "backup_value_chars": len(str(backup.get("backup_value") or "")),
        },
        "translations_register_called": False,
    }


def _real_write_execution_plan() -> list[str]:
    return [
        "Load all source reports.",
        "Re-validate product_id.",
        "Re-validate locale.",
        "Re-validate field=meta_title.",
        "Re-validate proposed_value <= 60 chars.",
        "Re-validate verified backup.",
        "Re-validate final human approval package.",
        "Re-validate final safe shell.",
        f"Re-validate dangerous flag: {shell.base.FUTURE_REQUIRED_FLAG}.",
        "Perform exactly one Shopify translationsRegister mutation.",
        "Immediately perform readback for the same product x locale x meta_title.",
        "Compare readback value exactly with proposed_value.",
        "If match, mark success and record local success report.",
        "If mismatch or API error, mark failure and record local failure report.",
        "Do not automatically rollback.",
        "Require separate rollback approval if needed.",
    ]


def _readback_plan() -> list[str]:
    return [
...
def _failure_handling_plan() -> list[str]:
    return [
...
def _rollback_approval_plan() -> list[str]:
    return [
        ...
        "Rollback cannot run automatically in Phase 12.1B.",
    ]


def _phase_12_1b_required_inputs() -> list[str]:
    return [
        "product_id",
        "locale",
        "field=meta_title",
        ...
        f"dangerous flag: {shell.base.FUTURE_REQUIRED_FLAG}",
        "an explicit Phase 12.1B execution ack that Phase 12.1A does not accept",
    ]


def _phase_12_1b_manual_approval_statement(scope: dict) -> str:
    return (
        "I understand Phase 12.1B may perform exactly one real Shopify translationsRegister write for "
        f"product {scope.get('product_id', '')}, locale {scope.get('locale', '')}, field {scope.get('field', '')}. "
        "I confirm the proposed value, verified backup, readback plan, and rollback approval plan. "
        "I understand Phase 12.1A does not write Shopify."
    )


def _phase_12_1b_forbidden_actions() -> list[str]:
    return [
        "batch mode",
        "full-store scan",
        "multiple products",
...
def _safety_summary(dangerous_flag_present: bool, final_safe_ack_present: bool, plan_ack_present: bool) -> dict:
    return {
        "execution_plan_only": True,
        "payload_preview_only": True,
        "dangerous_flag_present": dangerous_flag_present,
        "dangerous_flag_effective": False,
        "final_safe_shell_ack_present": final_safe_ack_present,
...
def _write_json_report(payload: dict) -> Path:
    ...
    json.loads(text)
    EXECUTION_PLAN_JSON_PATH.write_text(text, encoding="utf-8")
    json.loads(EXECUTION_PLAN_JSON_PATH.read_text(encoding="utf-8"))
    return EXECUTION_PLAN_JSON_PATH


def _write_html_report(payload: dict) -> Path:
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    EXECUTION_PLAN_HTML_PATH.write_text(_render_html_report(payload), encoding="utf-8")
    return EXECUTION_PLAN_HTML_PATH
...
def _render_html_report(payload: dict) -> str:
    ...
    summary_rows = "\n".join(
        ...
        for label, key in [
            ...
            ("Payload Preview Only", "payload_preview_only"),
    ...
    detail_rows = "\n".join(
        ...
        for label, value in [
            ...
            ("Translations Register Payload Preview", payload.get("translations_register_payload_preview", {})),
            ("Real Write Execution Plan", payload.get("real_write_execution_plan", [])),
            ("Readback Plan", payload.get("readback_plan", [])),
            ("Failure Handling Plan", payload.get("failure_handling_plan", [])),
            ("Rollback Approval Plan", payload.get("rollback_approval_plan", [])),
            ("Phase 12.1B Required Inputs", payload.get("phase_12_1b_required_inputs", [])),
            ("Phase 12.1B Manual Approval Statement", payload.get("phase_12_1b_manual_approval_statement", "")),
            ("Phase 12.1B Forbidden Actions", payload.get("phase_12_1b_forbidden_actions", [])),
...
<head>
  <meta charset="utf-8">
  <title>Shopify Single-Field Real Write Execution Plan</title>
  <style>
...
<body>
  <h1>Shopify Single-Field Real Write Execution Plan</h1>
  ...
  <ul>
    ...
    <li>The payload is a preview only and cannot execute a real Shopify write.</li>
  </ul>
</body>
</html>
"""
...
def _issue_summary(execution_plan_status: str, blocking_conditions: list[str]) -> str:
    if blocking_conditions:
        return "Single-field real write execution plan blocked: " + ", ".join(blocking_conditions)
    return (
        f"Single-field real write execution plan generated with status {execution_plan_status}. "
...
def _build_approval_message(payload: dict, json_path: Path, html_path: Path) -> str:
    return (
        "Shopify single-field real write execution plan generated.\n"
        ...
        f"Phase 12.1A plan ack present: {payload.get('phase_12_1a_plan_ack_summary', {}).get('ack_present')}\n"
        f"Phase 12.1A plan ack effective: {payload.get('phase_12_1a_plan_ack_summary', {}).get('ack_effective')}\n"
        f"Blocking conditions: {payload.get('blocking_conditions')}\n"
        "Execution plan JSON:\n"
        f"{json_path}\n\n"
        "Execution plan HTML:\n"
        f"{html_path}\n"
        "Execution plan only. No Shopify API call, command execution, readback, rollback, mutation, translationsRegister, or write was performed.\n\n"
        "Allowed actions only:\n"
        "Y / 1 = keep execution plan files\n"
